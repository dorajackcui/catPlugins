...
/** Owns serialized Chrome Debugger attachment and idle-detach lifecycle. */
export class DebuggerSession {
  ...
  async prepare(tabId: number): Promise<void> {
    await this.enqueueTransition(tabId, async () => {
      ...
      // A fresh attachment shows an infobar and resizes the editor page.
  ...
  private handleDetached(source: { tabId?: number }): void {
    ...
    if (
      ...
      attachment?.idleTimer !== undefined
    ) {
  ...
  private enqueueTransition(
    ...
    this.transitions.set(tabId, next.catch(() => undefined));
    return next;
  }
