...
(() => {
  // chrome-api.ts
  async function storageGet(keys2) {
  ...
  async function executeScript(tabId, files, options) {
    return new Promise((resolve, reject) => {
  ...
  function parseExcelBuffer(buffer, fileName) {
    const workbook = readSync(buffer, { type: "array", cellStyles: true });
    ...
    const visibleRows = rows.map((values, index) => ({
      rowNumber: index + 1,
      values
  ...
  function buildPreview(entries, segments) {
  ...
  function applyMemoqPreviewCorrection(preview) {
    const unmatchedIndex = [...preview.items].map((item, index) => ({ item, index })).reverse().find(({ item }) => item.status === "unmatched")?.index;
    if (unmatchedIndex === void 0) {
      return preview;
    }
  ...
  // background.ts
  var MEMOQ_URL_RE = /^https:\/\/memoq\.[^/]+\.net\/memoqweb\/webpm\/webtrans\//;
  var MEMSOURCE_JOB_URL_RE = /^https:\/\/cloud\.memsource\.com\/web\/job\/[^/]+\/translate(?:[/?#]|$)/;
  var MEMSOURCE_EDITOR_FRAME_URL_RE = /^https:\/\/editor\.memsource\.com\/twe\/translation\/job\/[^/?#]+/;
  function isPhraseEditorUrl(url) {
    if (!url) {
      return false;
    }
    return url.startsWith("https://app.phrase.com/editor/") || MEMSOURCE_JOB_URL_RE.test(url) || MEMOQ_URL_RE.test(url);
  }
  function isMemoqUrl(url) {
    return Boolean(url && MEMOQ_URL_RE.test(url));
  }
  function isMemsourceEditorFrameUrl(url) {
    return Boolean(url && MEMSOURCE_EDITOR_FRAME_URL_RE.test(url));
  }
  function finalizePreviewForTab(url, result) {
    if (!isMemoqUrl(url)) {
      return result;
    }
    return {
  ...
  function buildPreviewForTab(url, preview) {
    return isMemoqUrl(url) ? applyMemoqPreviewCorrection(preview) : preview;
  }
  async function ensurePhraseTab() {
  ...
  async function handleMessage(request) {
    switch (request.type) {
      ...
      case "RUN_PREVIEW": {
        ...
        const tab = await ensurePhraseTab();
        const response = await sendTabMessage(tab.id, { type: "CONTENT_SCAN" }, tab.frameId ? { frameId: tab.frameId } : void 0);
        if (!response.ok) {
